    ...
    if (fileList) {
        ...
        (async (): Promise<void> => {
            ...
            for (let i: number = 0; i < fileList.length; i++) {
                ...
                if (file) {
                    const modifiedImage: File = await preprocessImage(
                        file,
                        currentPreprocess.value
                    );
                    ...
                    const pattern = new RegExp(
                        arcanistNameGroup + parenGroup + bannerGroup + dateGroup,
                        'i'
                    );
                    const currentPulls: {
                        ArcanistName: string;
                    ...
                    const excludeLineInfo =
                        /(?:<\s*)?(\d+)\s*\/\s*(\d+)(?:\s*>)?|(Name\sType\sSummon\sTime)/;
                    text.value
                        .trim()
                        .split('\n')
                        .forEach((line) => {
                            ...
                            if (match) {
                                ...
                                let fuseResult: FuseResult<string>[] = arcanistFuse.search(
                                    arcanistName,
                                    {
                                        limit: 1
                                    }
                                ...
                                const isGoldenThread: boolean =
                                    fuseResult[0].item.includes('The Golden Thread');
                                ...
                                pull.Rarity = isGoldenThread
                                    ? 6
                                    : arcanists[fuseResult[0].refIndex].Rarity;

                                /* fuzzy match banner capture */
                                fuseResult = bannerFuse.search(
                                    match.groups?.BannerType.trim() || '',
                                    { limit: 1 }
                                );
                                pull.BannerType = fuseResult[0]
                                    ? bannerList[fuseResult[0].refIndex]
                                    : '';
                    ...
                    Object.keys(currentPullsMapping).forEach((ts) => {
                        ...
                        if (!timestampMapping[ts]) {
                            ...
                            const isExistComplete10Pulls = timestampMapping[ts].some(
                                (ary) => ary.length === 10
                            );
                            if (!isExistComplete10Pulls) {
                                ...
                                if (
                                    ...
                                    isEqualPull(
                                        pullsArray[0],
                                        currentPulls[currentPulls.length - pullsArray.length]
                                    )
                                ) {
            ...
            Object.keys(timestampMapping)
                ...
                .forEach((timestamp) => {
                    if (timestampMapping[timestamp].length === 1) {
                        ...
                        const firstHalf = timestampMapping[timestamp][0];
                        const secondHalf =
                            timestampMapping[timestamp][timestampMapping[timestamp].length - 1];
                        const fullLengh = firstHalf.length + secondHalf.length;
                        if (!isEqualPulls(firstHalf, secondHalf)) {
                            if (fullLengh <= 10) {
                                flattenPullsArrayByTimestamp[timestamp] = [
    ...
    <div class="container">
        <div class="flex pb-4">
            <h2 class="text-2xl text-white font-bold">
                {{ $t('summon-tracker') }}
                <span class="text-warning text-sm">{{ $t('please-read-tutorial') }}</span>
            </h2>
        </div>
        ...
        <div
            ...
            <button
                ...
                class="bg-gradient-to-br from-warning to-yellow-600 focus:ring-2 focus:outline-none focus:ring-green-200 hover:bg-gradient-to-bl text-white/90 font-bold py-2 px-4 rounded ml-2">
                {{ $t('ocr-import') }}
            </button>
                    ...
                    <form method="dialog">
                        <div class="tooltip" data-tip="Stable. Works on all OS.">
                            <button
                                ...
                                :disabled="isImporting"
                                class="bg-gradient-to-br from-warning to-yellow-600 focus:ring-2 focus:outline-none focus:ring-green-200 hover:bg-gradient-to-bl text-white/90 font-bold py-2 px-4 rounded">
                                Version 1
                            </button>
                        </div>
                    </form>
        ...
        <div class="flex flex-wrap justify-center space-x-5 pb-5 gap-y-5">
            <button
                ...
                class="text-white py-1 px-3 hover:bg-warning rounded-md"
            ...
            <div class="dropdown dropdown-bottom dropdown-end">
                ...
                <ul
                    tabindex="0"
                    class="dropdown-content z-[1] shadow bg-red-950 custom-border-light rounded-box space-y-2 p-4 mt-2">
                    <li>
                        <button
                            v-bind:class="{
