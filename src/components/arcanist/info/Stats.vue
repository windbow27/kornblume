...
const editingArcanist = computed(() => ({
  ...
  currentInsight: 0,
  currentLevel: 1,
  goalInsight: (props.arcanist.Rarity >= 5 ? nodesHighRarity : nodesLowRarity).value[selected.value]
    .insight,
  goalLevel: (props.arcanist.Rarity >= 5 ? nodesHighRarity : nodesLowRarity).value[selected.value]
    .level,
  currentResonance: 0,
  goalResonance: 0,
  frequency: [],
...
const currentStats = computed(
  () =>
    props.arcanist.Stats[
      editingArcanist.value.goalInsight + '-' + editingArcanist.value.goalLevel
    ] ?? []
);

console.log('currentStats', currentStats.value);
const hasStats = computed(() => currentStats.value.some((stat: number) => stat !== 0));
</script>
  ...
  <div class="px-2 h-full flex flex-col">
    ...
    <div class="px-8 lg:px-16 py-2">
      <div class="w-full text-white flex justify-between text-xs 3xl:text-base">
        ...
        <span v-if="props.arcanist.Rarity >= 5"
          ><img class="w-5" src="/images/items/common/insight3.webp"
        /></span>
      </div>
      <input
        id="level"
        type="range"
        min="0"
        :max="props.arcanist.Rarity >= 5 ? 4 : 3"
        class="range-slider"
        step="1"
        v-model="selected" />
      <div class="w-full text-white flex justify-between text-xs 3xl:text-base">
    ...
    <div v-if="hasStats" class="pt-2.5">
      <table class="table table-xs">
        <tr v-for="(attribute, index) in attributes" :key="index">
          <td class="text-white text-center">{{ attribute }}</td>
          <td class="text-white">{{ currentStats[index] + (index >= 5 ? '%' : '') }}</td>
        </tr>
      </table>
    </div>
    <div v-else class="pt-2.5 text-center text-sm text-gray-500 italic">
