...
const stageStore = useDataStore().stages2_8_greedy;
...
const arcanistIconList = computed(() => {
  ...
  let arcanists = arcanistStore.filter((arcanist) => {
    return (
      arcanist.Insight.some((insight) => insight.Material.includes(props.selectedMaterial.Name)) ||
      arcanist.Resonance.some((resonance) =>
        resonance.Material.includes(props.selectedMaterial.Name)
      )
    );
  });
...
<template>
  <div v-if="props.selectedMaterial" class="custom-box custom-border">
    <div class="space-y-1">
      <h2 class="text-white text-2xl font-bold py-1">
        {{ $t(props.selectedMaterial.Name) }}
      </h2>
      <p
        class=""
        :class="{
        ...
        {{ $t(props.selectedMaterial.Category) }}
      </p>
      <p
        class=""
        :class="{
      ...
      <div class="flex flex-col justify-center items-center">
        <img
          :src="getItemImagePathByMatl(props.selectedMaterial.Name)"
          alt="Material Image"
          class="w-32 h-32" />
  ...
  <div v-if="formulaItemList.length > 0" class="custom-box custom-border space-y-2.5">
    ...
    <div class="flex flex-wrap gap-x-2 gap-y-1 items-center justify-center">
      <div
        v-for="material in formulaItemList"
  ...
  <div v-if="arcanistIconList.length > 0" class="custom-box custom-border space-y-2.5">
    ...
    <div class="custom-item-list gap-x-2 gap-y-1 max-h-[calc(33vh)]">
      <ArcanistIconDisplay
        v-for="arcanist in arcanistIconList"
        :key="arcanist.Name"
        :arcanist="arcanist" />
    </div>
  </div>
