...
const TrackerEditor = defineAsyncComponent(() => import('./TrackerEditor.vue'));
...
const sixStarsPullsList = computed(() => {
    ...
    return sixStarPulls.map((pullNumber, index) =>
        index === 0 ? pullNumber : pullNumber - sixStarPulls[index - 1]
    );
});
...
const sixStarsPullsIndex = computed(() => {
    ...
    sixStarPulls.forEach((pullNumber, index) => {
        indexBasedOnPullNumber[pullNumber] =
            index === 0 ? pullNumber : pullNumber - sixStarPulls[index - 1];
    });
        ...
        for (const pull of reversedPulls) {
            if (pull.Rarity === 6) {
                if (nextPullShouldBeG) {
                    ...
                    if (
                        bannerIndex !== -1 &&
                        bannerRateUp[bannerIndex].includes(pull.ArcanistName)
                    ) {
...
<template>
    ...
    <div class="flex flex-col text-white p-4 gap-2 max-w-sm mx-auto">
            ...
            <div class="number">
                {{
                    pulls.filter((p) => p.Rarity === 6).length > 0
                        ? Math.floor(
                              (pulls.length - summonSinceLastSixStar) /
                                  pulls.filter((p) => p.Rarity === 6).length
                          )
                        : 0
                }}
            </div>
            ...
            <div class="number">
                {{
                    pulls.filter((p) => p.Rarity === 5).length > 0
                        ? Math.floor(pulls.length / pulls.filter((p) => p.Rarity === 5).length)
                        : 0
                }}
            </div>
        </div>
        <div v-if="props.text == $t('summary-limited')" class="flex justify-between">
            ...
            <div class="number">{{ winrate ? winrate : 0 }} %</div>
        </div>
        ...
        <div class="flex flex-col justify-between opacity-95" v-if="isError">
            ...
            <div class="text-error text-sm">
                <i18n-t keypath="wrong-timestamps">
                    <template #timestamps>
                        <span class="number text-sm">{{
                            wrongTimestamps.map(formatDate).join(', ')
                        }}</span>
                    </template>
                </i18n-t>
            </div>
    ...
    <div
        class="w-full items-center custom-gradient-gray-blue rounded border border-red-800 justify-center px-4 pt-4 pb-3">
        ...
        <div class="flex flex-wrap justify-center gap-x-10">
            <!-- Fix the key later -->
            <div
                v-for="(pull, index) in pulls.filter((p) => p.Rarity === 6)"
    ...
    <div class="pt-10 pb-4">
        ...
        <div class="flex justify-center items-center space-x-2 pt-4">
            <span :class="{ 'opacity-50': isEditing, 'text-white': true }"
                ><i class="fa-solid fa-eye"></i
            ></span>
            <div class="tooltip" :data-tip="isEditing ? $t('summon-history') : $t('summon-editor')">
                <input
                    type="checkbox"
                    class="toggle toggle-info [--tglbg:#121b31]"
                    v-model="isEditing" />
            </div>
            <span :class="{ 'opacity-50': !isEditing, 'text-white': true }"
    ...
    <div v-if="showSpinner" class="text-white text-center font-bold">
    ...
    <div v-if="!isEditing" class="flex flex-col overflow-x-auto hidden-scrollbar">
        ...
        <table class="table-auto max-w-2xl w-full text-white lg:mx-auto">
            ...
            <tbody>
                <tr
                    v-for="(pull, index) in filteredRarityPulls"
                    ...
                    <td class="flex items-center px-4 gap-x-3 whitespace-nowrap">
                        ...
                        <span v-if="indicators[pull.PullNumber] === 'L'" class="badge-indicator">
                            <div class="tooltip" :data-tip="$t('lose')">
                                <img src="/images/items/common/red-badge.webp" />
                            </div>
                        </span>
                        <span
                            v-else-if="indicators[pull.PullNumber] === 'W'"
                            class="badge-indicator">
                            <div class="tooltip" :data-tip="$t('win')">
                                <img src="/images/items/common/green-badge.webp" />
                            </div>
                        </span>
                        <span
                            v-else-if="indicators[pull.PullNumber] === 'G'"
                            class="badge-indicator">
                            <div class="tooltip" :data-tip="$t('guaranteed')">
                                <img src="/images/items/common/yellow-badge.webp" />
                            </div>
                        </span>
                    </td>
